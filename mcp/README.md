...
- Linux amd64, Node 22+, Docker access, and disk space for the upstream image/index.
...
3. Search chunks may omit context. Confirm the full capability's prerequisites, limits, `wholeClaim`, status and evidence before making a claim. Partial, failed and untested are never verified.
...
Only the indexer receives the generated documents mount, read-only. The reader receives no document/repository/home mount and has file fetching disabled, plus an empty outbound allowlist. Its upstream tool list still includes `fetch_url`, but network/file requests are denied. Write/job tools are unavailable in read-only mode. The index data mount remains writable for SQLite/runtime housekeeping; read-only here describes available application operations, not an immutable filesystem database. Stop readers before changing operator-owned index files.
...
The Docker test uses synthetic text in a uniquely named temporary directory, leaves it for inspection, and removes its temporary containers. It exercises indexing/search, stdio MCP initialization/search, unavailable writes, and denied network/file fetching. It does not use any live Kamiwaza API, cluster credential, or frozen verification resource. Pull the pinned image before running it; execution uses `--pull=never`.
