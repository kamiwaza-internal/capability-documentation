...
test('MCP-LIVE-1 pinned upstream indexes and retrieves synthetic content with no network', async () => {
  ...
  publication.releases[0].capabilities[0].conditions.push(...Array.from({length:12},(_,i)=>`Control ${i}: `+'synthetic filler '.repeat(400)));
  publication.releases[0].capabilities[0].limits.push('FINAL CONTROL: never infer a verified result');
  const dir=join(root,'bundle'); const bundle=prepareBundle(publication,'synthetic-r1',dir);
  ...
  indexBundle(publication,dir,args => {
  ...
  const containerName='capability-smoke-'+root.split('-').at(-1).toLowerCase();
  const servingArgs=readerArgs(publication,dir); servingArgs.splice(1,0,'--name',containerName);
  const child=spawn('docker',servingArgs,{stdio:['pipe','pipe','pipe']});
  ...
  try {
    ...
    for (const expected of ['synthetic-r1']) {
    ...
    if (names.includes('fetch_url')) {
      const full=await request('tools/call',{name:'fetch_url',arguments:{url:'file:///input/synthetic.search.txt'}});
      assert.ok(!full.error && !full.result?.isError,JSON.stringify(full));
      for (const expected of ['No product evidence','synthetic-r1','amber','publicationRevision','FINAL CONTROL', ...Array.from({length:12},(_,i)=>`Control ${i}:`)]) {
        assert.ok(JSON.stringify(full).includes(expected),JSON.stringify(full));
      }
    ...
    child.stdin.end();
    const removed=spawnSync('docker',['rm','-f',containerName],{encoding:'utf8',timeout:15000});
